...
namespace hybridclr
{
    ...
    namespace
    {
        ...
        static bool ReadPublishedMvid(const Il2CppImage* image, uint32_t imageId, std::string& mvid)
        {
            ...
            // Never walk MetadataModule here: it may expose a private staged
            // image and would invalidate this publication-only observation.
            ...
            bool published = false)
        {
            ...
            out << "{\"nativeAssemblyId\":" << il2cpp::vm::AssemblyShadowDiagnostics::Quote(NativePointer(assembly))
                ...
                << ",\"identityKey\":" << il2cpp::vm::AssemblyShadowDiagnostics::Quote(
                    ...
                    "|" + fullName + "|" + NativePointer(assembly) + "|" + NativePointer(image) +
    ...
    int32_t RuntimeApi::GetH1CountDiagnosticsJson(Il2CppString** json)
    {
        ...
        try
        {
            ...
            uint64_t shadow = 0;
            uint64_t reserved = 0;
            // The unfixed reproduction intentionally retains the original
            // snapshot helper, which acquires g_MetadataLock internally.
